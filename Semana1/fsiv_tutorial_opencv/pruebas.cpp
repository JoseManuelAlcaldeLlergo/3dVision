...
const cv::String keys =
    ...
    "{@image2         |<none>|image to show.}" //argumento sin valor por defecto.
    ;
...
main (int argc, char* const* argv)
{
  ...
  try {    
      ...
      cv::String img_name2 = parser.get<cv::String>("@image2"); //Obtenermos el argumento @image.
      ...
      cv::Mat img2 = cv::imread(img_name2, cv::IMREAD_ANYCOLOR);
      ...
      img(cv::Range(0,100),cv::Range(0,100)) = img2(cv::Range(0,100),cv::Range(0,100));
      cv::imshow("IMG", img);
      std::cout << "Pulsa ESC para salir." << std::endl;
      while ((cv::waitKey(0) & 0xff) != 27); //Hasta que no se pulse la tecla ESC no salimos.

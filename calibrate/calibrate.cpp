...
main (int argc, char* const* argv)
{
    ...
    try {        
        ...
        else
        {
            ...
            std::vector<cv::Point3f> _3d_corners;
            ...
            for(size_t v; v<input_fnames.size(); v++){
                ...
                //comprobar si todaslas imagene son del mismo tamaño
            ...
            float error = fsiv_calibrate_camera(_2dpoints,_3dpoints, camera_size, cameraMatrix, distMatrix, &rvecs, &tvecs);
            std::cout<<"Error de reproyección: "<<error<<std::endl;
            for(size_t v=0; v<input_fnames.size(); v++){
                cv::Mat img = cv::imread(input_fnames[v]);
                fsiv_draw_axes(img,cameraMatrix,distMatrix,rvecs[v],tvecs[v],square_size, 3);
                cv::imshow("EJES",img);
                cv::waitKey(0);
            }
            ...
            fsiv_calibrate_camera(_2dpoints, _3dpoints, camera_size, cameraMatrix,distMatrix);
